...
def update_mode(*args):
    ...
    if mode == "Enter Resolution":
        frame_resolution.grid(row=4, column=0, columnspan=3, padx=5, pady=5, sticky="w")
    elif mode == "Scale Percentage":
        frame_scale.grid(row=4, column=0, columnspan=3, padx=5, pady=5, sticky="w")
    elif mode == "Target File Size":
        frame_target.grid(row=4, column=0, columnspan=3, padx=5, pady=5, sticky="w")
...
def convert_gif():
    ...
    if os.path.exists(output_file):
        try:
            ...
            messagebox.showerror("Error", f"Failed to remove existing output file:\n{e}")
            return
    ...
    try:
        if mode == "Enter Resolution":
            ...
            for i in range(iterations):
                mid = (low + high) / 2
                temp_gif = tempfile.NamedTemporaryFile(delete=False, suffix=".gif").name
                ...
                subprocess.run(cmd_palette_iter, check=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
                
                cmd_gif_iter = [
                    "ffmpeg", "-y", "-i", input_file, "-i", temp_palette,
                    "-filter_complex", filter_complex_iter, temp_gif
                ]
                subprocess.run(cmd_gif_iter, check=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
...
tk.Label(root, text="Input GIF File:").grid(row=0, column=0, padx=5, pady=5, sticky="e")
...
tk.Label(root, text="Resize Mode:").grid(row=3, column=0, padx=5, pady=5, sticky="e")
...
tk.Label(frame_resolution, text="Height:").grid(row=0, column=2, padx=5, pady=5)
...
tk.Label(frame_scale, text="Scale Percentage:").grid(row=0, column=0, padx=5, pady=5)
...
tk.Label(frame_target, text="Target File Size (MB):").grid(row=0, column=0, padx=5, pady=5)
target_slider = tk.Scale(frame_target, from_=0.1, to=20, resolution=0.1, orient=tk.HORIZONTAL)
...
# Convert button.
tk.Button(root, text="Convert", command=convert_gif).grid(row=5, column=1, padx=5, pady=15)
