...
class GifToolsApp:  
    def __init__(self, master):  
        ...
        self.add_text_button = ttk.Button(frame, text="Add Text to GIF", command=lambda: run_script("AddTextToGif.py"), bootstyle="success")  
        self.add_text_button.pack(pady=5, fill=X)  

        self.compress_button = ttk.Button(frame, text="Compress GIF", command=lambda: run_script("CompressGif.py"), bootstyle="info")  
        self.compress_button.pack(pady=5, fill=X)  

        self.convert_button = ttk.Button(frame, text="Convert MP4 to GIF", command=lambda: run_script("ConvertMP4toGIF.py"), bootstyle="warning")  
        self.convert_button.pack(pady=5, fill=X)  

        self.edit_frames_button = ttk.Button(frame, text="Edit GIF Frames", command=lambda: run_script("EditGifFrames.py"), bootstyle="primary")  
        self.edit_frames_button.pack(pady=5, fill=X)  

        self.resize_button = ttk.Button(frame, text="Resize GIF", command=lambda: run_script("ResizeGif.py"), bootstyle="light")  
